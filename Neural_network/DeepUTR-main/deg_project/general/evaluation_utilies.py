...
def evaluate_slope_test(y_predicted, y_observed, compute_mean_and_std=True):
    ...
    return slope_test(predicted_slops, observed_slops, compute_mean_and_std,
                      AE_values)


def slope_test(predicted_slops, observed_slops, compute_mean_and_std,
               AE_values=None):
...
def evaluate_linear_or_nonlinear_subset(y_predicted, y_observed,
                                        subset='nonlinear',
    ...
    return evaluate_model(y_predicted[:, 1:], y_observed[:, 1:],
                          compute_mean_and_std)  # perform the evaluation without the initial point.


###############################evaluate slope test for linear fit subset#######################################
def evaluate_slope_test_for_linear_or_nonlinear_subset(y_predicted, y_observed,
                                                       subset='linear',
                                                       compute_mean_and_std=True,
                                                       R2_threshold=0.7):
...
def drop_linear_or_nonlinear_subset(y_predicted, y_observed, subset_to_retain,
                                    R2_threshold):
    y_observed = np.squeeze(np.asarray(y_observed))
