...
def savePattern(patten, filename, LEN=70):
    raw_data = {'Pos': np.arange(len(patten)) + 1, 'A': patten[:, 0],
                'C': patten[:, 1], 'G': patten[:, 2], 'T': patten[:, 3]}
...
def run_modisco(hyp_impscores, impscores, onehot_data, null_distribution):
    ...
    activity_patterns = \
        np.array(hdf5_results['metaclustering_results']['attribute_vectors'])[
            np.array(
                [x[0] for x in sorted(
                    enumerate(hdf5_results['metaclustering_results'][
                                  'metacluster_indices']),
                    key=lambda x: x[1])])]
    ...
    for metacluster_name in metacluster_names:
        ...
        for i, pattern_name in enumerate(all_pattern_names):
            ...
            pattern = metacluster_grp["seqlets_to_patterns_result"]["patterns"][
                pattern_name]
            print("total seqlets:",
                  len(pattern["seqlets_and_alnmts"]["seqlets"]))
...
def generate_dataset_importance_scores(model, sequences_fetures,
                                       intial_features, target_range,
                                       batch_size=25,
                                       to_create_null_distribution=True):
    # compute both importance score (ex) and hypothetical importance score (hyp_ex)
    ex_list, hyp_ex_list = compute_impratnce_scores(model, sequences_fetures,
                                                    intial_features,
                                                    target_range, batch_size)

    # create null distribution
    null_distribution = create_null_distribution(model, sequences_fetures,
                                                 intial_features, target_range,
                                                 batch_size,
                                                 to_create_null_distribution)
...
def compute_impratnce_scores(model, sequences_fetures, intial_features,
                             target_range, batch_size):
    ex_list = []
    hyp_ex_list = []
    part_nums = (
                        len(sequences_fetures) // batch_size) + 1  # devide the calculation acordding to the bach size
    for sequences_fetures_part, intial_features_part in tqdm(
            zip(np.array_split(sequences_fetures, part_nums),
                np.array_split(intial_features, part_nums))):
        for i in range(len(model)):
            ex_to_add, hyp_ex_to_add = get_integrated_gradients(model[i],
                                                                sample_inputs=[
                                                                    sequences_fetures_part,
                                                                    intial_features_part],
                                                                target_range=target_range,
                                                                multiple_samples=True)
            ex_to_add, hyp_ex_to_add = ex_to_add[0], hyp_ex_to_add[
                0]  # [0] for taking only sequence importance
...
def create_null_distribution(model, sequences_fetures, intial_features,
                             target_range, batch_size,
                             to_create_null_distribution=True):
    if to_create_null_distribution:
        null_distribution = []
        sequences_fetures_for_dinuc_shuffle, _, intial_features_dinuc_shuffle, _ = train_test_split(
            sequences_fetures, intial_features, test_size=0.5, random_state=42)
        for i in range(len(sequences_fetures_for_dinuc_shuffle)):
            sequences_fetures_for_dinuc_shuffle[
                i] = modisco_utilies.dinuc_shuffle(
                sequences_fetures_for_dinuc_shuffle[i], seed=42)

        part_nums = (len(sequences_fetures_for_dinuc_shuffle) // batch_size) + 1
        for sequences_fetures_for_dinuc_shuffle_part, intial_features_dinuc_shuffle_part in tqdm(
                zip(np.array_split(sequences_fetures_for_dinuc_shuffle,
                                   part_nums),
                    np.array_split(intial_features_dinuc_shuffle, part_nums))):
            for i in range(len(model)):
                ex_to_add, _hyp_ex_to_add = get_integrated_gradients(model[i],
                                                                     sample_inputs=[
                                                                         sequences_fetures_for_dinuc_shuffle_part,
                                                                         intial_features_dinuc_shuffle_part],
                                                                     target_range=target_range,
                                                                     multiple_samples=True)
                ex_to_add = ex_to_add[
                    0]  # [0] for taking only sequence importance
...
def generate_modisco_dataset(model_path, seq_path, labels_path_minus,
                             labels_path_plus, model_id, model_type,
                             data_type, target_range=None,
                             test_validation_train_or_all_set='test',
                             save_path=None, index_for_split=None,
                             batch_size=256):
    # load model or list of models
    if type(model_path) is list:
        model = [tf.keras.models.load_model(model_path_item, custom_objects={
            'tf_pearson': NN_utilies.tf_pearson}) for model_path_item in
                 model_path]
    else:
    ...
    data_sets = NN_load_datasets.load_dataset_model_type(seq_path=seq_path,
    ...
    if (test_validation_train_or_all_set == 'all'):
        ...
        if (test_validation_train_or_all_set == 'test'):
            ...
            (initial_values_features, one_hot_features,
             _) = validation_set_wrapper
            ...
            (initial_values_features_v, one_hot_features_v,
             _) = validation_set_wrapper
            (
                initial_values_features_t, one_hot_features_t,
                _) = test_set_wrapper
            initial_values_features, one_hot_features = np.concatenate((
                initial_values_features_v,
                initial_values_features_t)), np.concatenate(
                (one_hot_features_v, one_hot_features_t))
        else:
            raise ValueError('invalid test_validation_train_or_all_set')

    onehot_data, impscores, hyp_impscores, null_distribution = generate_dataset_importance_scores(
        model, one_hot_features, initial_values_features, target_range,
        batch_size)

    # save the resutls in neeeded
    if (save_path is not None):
        Path(save_path).mkdir(parents=True, exist_ok=True)
        general_utilies.pickle.dump(
            [hyp_impscores, impscores, onehot_data, null_distribution], open(
                save_path + 'modisco_input_' + model_id + '_' + data_type + '_' + str(
                    'all_outputs' if target_range is None else target_range) + '_' + test_validation_train_or_all_set + '_set.sav',
                'wb'))
