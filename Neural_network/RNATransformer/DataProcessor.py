...
class Dataprocessor:
    def __init__(self, sequences_file=None, deg_rates_file=None,
        ...
        self.sequence = self.generate_one_hot_encoding_matrix(
            sequences_df.values)

        # Compute degradation rates for all sequences
        deg_rates = self.compute_LR_slopes(deg_rates_df.values)
        self.deg_rates = torch.tensor(deg_rates, dtype=torch.float32)

        if split_ids_file:
            indices_df = pd.read_csv(split_ids_file)

            self.train_set_seq, self.train_set_deg = self._prepare_data(
                indices_df['train_ids'])
            self.validation_set_seq, self.validation_set_deg = self._prepare_data(
                indices_df['validation_ids'].dropna())
            self.test_set_seq, self.test_set_deg = self._prepare_data(
                indices_df['test_ids'].dropna())
    ...
    def _prepare_data(self, indices):
        ...
        one_hot_seq = self.sequence[indices]
        deg_rates = self.deg_rates[indices]
        return one_hot_seq, deg_rates
    ...
    def get_train_set(self) -> tuple[torch.tensor, torch.tensor]:
        return self.train_set_seq, self.train_set_deg

    def get_validation_set(self) -> tuple[torch.tensor, torch.tensor]:
        return self.validation_set_seq, self.validation_set_deg

    def get_test_set(self) -> tuple[torch.tensor, torch.tensor]:
        return self.test_set_seq, self.test_set_deg
