...
#     "./data_for_linear_reg/models/filtered_data_5utr_40A_model.joblib")
...
lasso_weights_df = pd.read_csv(
    "../linear_regression/data_for_linear_reg/weights_data"
    "/most_significant_kmers.csv")
lasso_weights_df.set_index("kmers", inplace=True)
    ...
    for pos in seq_weights.keys():
        for neighbor in [-2, -1, 1, 2]:
            if pos + neighbor >= 0 and pos + neighbor < len(sequence):
                seq_weights[pos] += (preprocessed_weights[pos + neighbor]) / (
                        (abs(neighbor) + 1) ** 2)
...
for sample in plots_data.keys():
        plot_sequence_weights(plots_data[sample],
                              list(sequences.loc[sample]['seq']),
                              f"{sample} - Lasso",
                              f'../results/5utr_k_mers_plots/sequence'
                              f'_{sample}_lasso.png',
                              max_val)
